...
public class ArrowGame : MonoBehaviour {
	...
	*/
	...
	if (Input.touchCount > 0) {
		//up
		if ((Input.GetTouch (0).deltaPosition.y > 0 && Input.GetTouch (0).deltaPosition.x < .3 && Input.GetTouch (0).deltaPosition.x > -0.3)) {						//checks the card and input
			if (arrows.Count >= 1) {								//if the top of the queue matches input
				if (arrows [0].type == 0) {								//success, else damages you
					correctInput ();
				} else {
		...
		//down
		if ((Input.GetTouch (0).deltaPosition.y < 0 && Input.GetTouch (0).deltaPosition.x < .3 && Input.GetTouch (0).deltaPosition.x > -.3)) {
			if (arrows.Count >= 1) {
				if (arrows [0].type == 1) {
					correctInput ();
				} else {
		...
		//left
		if ((Input.GetTouch (0).deltaPosition.x < 0 && Input.GetTouch (0).deltaPosition.y < .3 && Input.GetTouch (0).deltaPosition.y > -.3)) {
			if (arrows.Count >= 1) {
				if (arrows [0].type == 2) {
					correctInput ();
				} else {
